...
export async function downloadOptionContracts(): Promise<Contract[]> {
  ...
  try {
    ...
    const result: Contract[] = records
      .filter(
        (item) => item.Instrument == "14" && item.ScripName === "BANKNIFTY"
      )
      .map((item) => ({
        name: item.Name,
        lot: Number(item.lot),
    ...
    const redis = await client.multi();

    await redis.rpush(
      "option_chain",
      ...result.map((item) => JSON.stringify(item))
    );
    await redis.expire("option_chain", 7200);
    await redis.exec();
...
export function findContract(
  ...
  const filteredList = contractList.filter(
    (item: any) =>
      item.strikePrice === strikePrice && item.optionType === optionType
  );
  const sortedList = filteredList.sort(
    (a: any, b: any) => a.expiryTs - b.expiryTs
  );
